...
int main() {
	...
	// ÍNDICES
	//
	// Define a ordem em que os vértices devem ser desenhados.
	// ========================================================
	GLuint indices[] =
	{
		0, 1, 2,
	...
	glm::vec4 lightColor = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f); // Cor da luz (branca)

	glm::vec3 lightPos(0.5f, 0.5f, 0.5f); // Posição da luz
	glm::mat4 lightModel = glm::mat4(1.0f); // Matriz de modelo da luz
	lightModel = glm::translate(lightModel, lightPos); // Translada a luz para a posição desejada

	glm::vec3 pyramidPos(0.0f, 0.0f, 0.0f); // Posição da pirâmide
	glm::mat4 pyramidModel = glm::mat4(1.0f); // Matriz de modelo da pirâmide
	pyramidModel = glm::translate(pyramidModel, pyramidPos); // Translada a pirâmide para a posição desejada
	...
	Texture planksSpec(
		"assets/textures/planksSpec.png",
		GL_TEXTURE_2D,
		1,
		GL_RED,
		GL_UNSIGNED_BYTE
	...
	// Variáveis para calcular o tempo entre frames.
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// Calcula o tempo entre frames para movimentação suave da câmera.
